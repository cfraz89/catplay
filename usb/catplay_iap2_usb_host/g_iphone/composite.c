                      ...
                      const struct usb_ctrlrequest *ctrl, unsigned number)
{
    ...
    for (tmp = 0; tmp < MAX_CONFIG_INTERFACES; tmp++)
    {
        ...
        for (; *descriptors; ++descriptors)
        {
            ...
            addr = ((ep->bEndpointAddress & 0x80) >> 3) | usb_endpoint_num(ep);
    ...
    {
    } // C2A usb_gadget_set_selfpowered(gadget);
    ...
    unknown:
            ...
            ctrl->bRequest == cdev->b_webusb_vendor_code)
        {
            ...
            landing_page_length = strnlen(cdev->landing_page,
                                          sizeof(url_descriptor->URL) - WEBUSB_URL_DESCRIPTOR_HEADER_LENGTH + landing_page_offset);
            ...
            url_descriptor->bLength = landing_page_length - landing_page_offset + WEBUSB_URL_DESCRIPTOR_HEADER_LENGTH;
        ...
        if (cdev->config)
        {
            list_for_each_entry(f, &cdev->config->functions, list) if (f->req_match &&
                                                                       f->req_match(f, ctrl, false)) goto try_fun_setup;
        }
        else
        {
            struct usb_configuration *c;
            list_for_each_entry(c, &cdev->configs, list)
                list_for_each_entry(f, &c->functions, list) if (f->req_match &&
                                                                f->req_match(f, ctrl, true)) goto try_fun_setup;
        }
        f = NULL;
                                      ...
                                      const struct usb_device_descriptor *old)
{
    ...
    *new = *old;
    ...
    if (bcdDevice)
        ...
        new->bcdDevice = 0x1602; /* C2A */ /* cpu_to_le16(get_default_bcdDevice()) */
    ;
    if (iSerialNumber)
 ...
 */
int c2a_usb_composite_probe(struct usb_composite_driver *driver)
{
    ...
    if (driver->udc_name)
    {
        pr_info("iPhone: detected UDC override: %s\n", *driver->udc_name);
        gadget_driver->udc_name = *driver->udc_name; // C2A
    }
